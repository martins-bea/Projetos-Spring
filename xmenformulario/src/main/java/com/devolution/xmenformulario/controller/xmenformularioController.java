...
@RestController // Define que essa é a camada controller. Controller são os botões, e abaixo
				// vamos criar as opções disponiveis dele.
@RequestMapping("/mutantes") // Faz o mapeamento do protocolo http.
@CrossOrigin("*") // Garante que a requisição será executada no endereço em questão.
public class xmenformularioController {
	@Autowired // Controla o objeto criado abaixo, no caso xmenformularioRepository, para
				// economizar memoria e melhorar o desempenho.
	private xmenformularioRepository repositorioXmen;

	
	//GetMapping - Busca e mostra os dados de acordo com o parametro.
	@GetMapping
	...
	@GetMapping("nivelpoder/{nivelPoder}")
	public List<xmenformulario> GetByNivelPoder(@PathVariable int nivelPoder) {
		return repositorioXmen.findByNivelPoderGreaterThan(nivelPoder);
	}
